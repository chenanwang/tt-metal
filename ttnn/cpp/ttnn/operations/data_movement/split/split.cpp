...
std::vector<ttnn::Tensor> split(
    ...
    // For sharded inputs with no explicit output config: default to DRAM interleaved
    // to avoid shard-spec incompatibility when output shapes differ from input.
    ...
    // Convert sharded inputs to DRAM before the split operation.
    // Both the fast-path device kernel and the slice fallback require
    // interleaved tensors.
    const Tensor working_input = input_tensor.memory_config().is_sharded()
                                     ? ttnn::to_memory_config(input_tensor, ttnn::DRAM_MEMORY_CONFIG)
                                     : input_tensor;

    // Use interleaved memory config for the actual split operation
    // to avoid shard-spec conflicts with intermediate shapes.
    const MemoryConfig op_mem_cfg = desired_mem_cfg.is_sharded() ? ttnn::DRAM_MEMORY_CONFIG : desired_mem_cfg;
    ...
    // Re-shard outputs if user explicitly requested a sharded output memory config.
    if (desired_mem_cfg.is_sharded()) {
        for (auto& t : results) {
            t = ttnn::to_memory_config(t, desired_mem_cfg);
        }
    }
