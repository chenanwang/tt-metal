...
// pre-add x + y
    ...
    mul_tiles_init(cb_in, cb_col_mask);
    ...
    for (uint32_t i = 0; i < block_h; i++) {
        for (uint32_t wt = 0; wt < block_w; wt++) {
            ...
            tile_regs_acquire();
            mul_tiles(cb_in, cb_col_mask, wt + index_h_offset, mask_idx, 0);
            tile_regs_commit();
            tile_regs_wait();
    ...
    reconfig_data_format_srcb(cb_col_mask, cb_scaler);
    constexpr uint32_t cb_ex_reduce_input = cb_mask_scratch;
#else
    constexpr uint32_t cb_ex_reduce_input = cb_in;
#endif
    // E[x],
    ...
    mul_tiles_init(cb_xmm, cb_col_mask_packed);
    for (uint32_t t = 0; t < num_tiles_per_block; t++) {
        const uint32_t wt = t % block_w;
        cb_xmm_obj.wait_front(1);
        tile_regs_acquire();
        mul_tiles(cb_xmm, cb_col_mask_packed, 0, wt, 0);
        tile_regs_commit();
        cb_xmm_obj.pop_front(1);
        cb_xmm_obj.reserve_back(1);
        tile_regs_wait();
        pack_tile(0, cb_xmm);
        cb_xmm_obj.push_back(1);
        tile_regs_release();
    }
    cb_xmm_obj.wait_front(num_tiles_per_block);
#endif
    constexpr uint32_t cb_sq_input = cb_xmm;
    ...
    reconfig_data_format(cb_xmm2, cb_col_mask_packed);
    mul_tiles_init(cb_xmm2, cb_col_mask_packed);
    for (uint32_t t = 0; t < num_tiles_per_block; t++) {
        const uint32_t wt = t % block_w;
        cb_xmm2_obj.wait_front(1);
        tile_regs_acquire();
        mul_tiles(cb_xmm2, cb_col_mask_packed, 0, wt, 0);
        tile_regs_commit();
        cb_xmm2_obj.pop_front(1);
        cb_xmm2_obj.reserve_back(1);
        tile_regs_wait();
        pack_tile(0, cb_xmm2);
        cb_xmm2_obj.push_back(1);
        tile_regs_release();
    }
